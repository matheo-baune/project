  ...
  <div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="group in groups" :key="group.id" class="bg-white overflow-hidden shadow-sm rounded-lg">
          ...
          <div class="bg-gray-50 px-4 py-4 sm:px-6">
            <div class="flex justify-between">
              <button 
                ...
                class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 focus:outline-hidden focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
                View Members
              </button>
    ...
    <!-- View Members Modal -->
    <BaseModal :model-value="showMembersModal" @update:modelValue="val => (showMembersModal = val)">
      <template #header>
        <div class="flex items-center gap-3">
          <div class="mx-auto sm:mx-0 shrink-0 flex items-center justify-center h-10 w-10 rounded-full bg-indigo-100">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
            </svg>
          </div>
          <h3 class="text-lg leading-6 font-medium text-gray-900">{{ selectedGroup ? selectedGroup.name + ' Members' : 'Group Members' }}</h3>
        </div>
      </template>
      ...
      <template #footer>
        <UiButton variant="secondary" @click="showMembersModal = false">Close</UiButton>
      </template>
    </BaseModal>

    <!-- Delete Confirmation Modal -->
    <BaseModal :is-open="showDeleteModal" :title="'Delete Group'" @close="showDeleteModal = false">
      <template #header>
        <div class="flex items-center gap-3">
          ...
          <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Delete Group</h3>
        </div>
      </template>
