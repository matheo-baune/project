<template>
    <div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <!-- Users Table -->
            <div v-else class="bg-white shadow-sm overflow-hidden sm:rounded-lg">
                        ...
                        <tr v-for="user in users" :key="user.id">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="shrink-0 h-10 w-10 rounded-full overflow-hidden">
        ...
        >
            <template #header>
                <div class="flex items-center gap-3">
                    <div
                        class="mx-auto sm:mx-0 shrink-0 flex items-center justify-center h-10 w-10 rounded-full bg-indigo-100">
                    ...
                    <h3 class="text-lg leading-6 font-medium text-gray-900">
                        {{ showEditModal ? 'Edit User' : 'Create New User' }}</h3>
                </div>
            </template>

            <form id="user-form" @submit.prevent="handleSubmitUser">
                <!-- First name -->
                <div>
                    <label for="firstname" class="block text-sm font-medium text-gray-700">First name *</label>
                    <div class="mt-1">
                        <input type="text" name="firstname" id="firstname" v-model="userForm.firstname" required
                ...
                <div class="mt-4">
                    ...
                    <div class="mt-1">
                        <input type="text" name="lastname" id="lastname" v-model="userForm.lastname" required
                               class="shadow-xs focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md"
                               placeholder="Doe"/>
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <div class="flex items-center">
                        <input id="admin-role" name="admin-role" type="checkbox" v-model="userForm.isAdmin"
                               class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded-sm"/>
                        <label for="admin-role" class="ml-2 block text-sm text-gray-900">Admin privileges</label>
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Admins have full access to all system features and management
                        capabilities.</p>
                </div>
            </form>

            <template #footer>
                <UiButton form="user-form" type="submit" variant="primary"
                          :disabled="modalLoading || !userForm.firstname || !userForm.lastname || !userForm.email || (!showEditModal && !userForm.password)">
                    <template #icon>
                        <svg v-if="modalLoading" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white"
                ...
                <UiButton class="sm:ml-3 sm:mt-0 mt-3" variant="secondary" type="button" @click="closeModals">
                    Cancel
                </UiButton>
            </template>
        </BaseModal>
        ...
        <BaseModal :model-value="showDeleteModal" @update:modelValue="val => (showDeleteModal = val)"
                   :title="'Delete User'">
            <template #header>
                <div class="flex items-center gap-3">
                    <div
            ...
            <p class="text-sm text-gray-600">Are you sure you want to delete this user? This action cannot be
                undone.</p>

            <template #footer>
                <UiButton variant="primary" class="bg-red-600 hover:bg-red-700 focus:ring-red-500"
                          :disabled="modalLoading" @click="confirmDeleteUser">
                    <template #icon>
                        <svg v-if="modalLoading" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white"
                             xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                ...
                <UiButton class="sm:ml-3 sm:mt-0 mt-3" variant="secondary" type="button"
                          @click="showDeleteModal = false">{{ t('common.cancel') }}
                </UiButton>
            </template>
        </BaseModal>
    </div>
...
const handleSubmitUser = async () => {
    if (!userForm.value.lastname || !userForm.value.email || (!showEditModal.value && !userForm.value.password)) return;
        ...
        if (showEditModal.value) {
            ...
            if (index !== -1) {
                users.value[index] = {
                    ...
                    avatar:
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(userForm.value.lastname)}&background=${userForm.value.isAdmin ? 'FF5722' : '6366F1'}&color=fff`
                };
            }
        } else {
            // Create new user
